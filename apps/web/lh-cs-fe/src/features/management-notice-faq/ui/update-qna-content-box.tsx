...
// import { DialogPopup, ModalButton } from '@/shared/ui';
...
import { usePublicQnaById, useUpdateQna } from '@/shared/api/hooks/qna-hooks';
import {
  NoticeAttachments,
...
interface UpdateQnaContentBoxProps {
  id: string;
  handleUndoPage: () => void;
}
...
const UpdateQnaContentBox = ({
  id,
  handleUndoPage,
}: UpdateQnaContentBoxProps) => {
  // 공지사항 생성 mutation hook
  const { data: qnaItemData } = usePublicQnaById(id);
  ...
  const { mutate: updateQna, isPending: isSubmitting } = useUpdateQna();
  ...
  const handleSubmit = async () => {
    ...
    updateQna(
      ...
      {
        onSuccess: () => {
          toast.showSuccess('게시물이 수정되었습니다.');
        ...
        onError: (error) => {
          const message =
            error instanceof Error ? error.message : 'QnA 수정에 실패했습니다.';
...
export default UpdateQnaContentBox;
